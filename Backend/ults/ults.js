...
var vULTSEntityMapByOID = new Map();
var vULTSPersonMap = new Map();
var vULTSPersonMapByEmployeeID = new Map();
...
async function loadAreaData() {
    if (!vLoadingAreaData) {
        ...
        try {
            ...
            console.error("ULTS:LOADAREAS: ERROR:" + e); // should contain code (exit code) and signal (that caused the termination).
    ...
    if (!vLoadingLocationData) {
        ...
        try {
            ...
            vFLTLocations.features.forEach((value, index) => {
                ...
                value.sendTime = value.properties.reported_at;//vSendTime;
                value.properties.position_source = "mstarfleet";
...
//Initial load of Location data.
async function loadPersonDetails(vEmployeeID) {
	vPersonDetails={};
	try {
    	console.log("ULTS:LOADPERSONDETALS: Requesting persons details...");
        var vURL='http://localhost:4990/api/getPTFIDetailsEmployee?employee_id='+vEmployeeID+'';
		params = {
            method: 'GET',
            url: vURL,
            json: true,
		...
		const data = await axios(params);
        console.log('ULTS:LOADPERSONDETAILS: Response['+JSON.stringify(data.data)+'].');
        vPersonDetails = data.data;
    } catch (e) {
        console.error("ULTS:LOADPERSONDETAILS: ERROR:" + e); // should contain code (exit code) and signal (that caused the termination).
    }
	return vPersonDetails;
}
                ...
                for (const recordset of result.recordsets) {
                    ...
                    for (const value of recordset) { //.forEach((value,index) => {
                        try {
                            ...
                            vPersonelTracker.sendTime = vPersonelTracker.properties.reported_at;//vSendTime;
                            ...
                            if (vULTSEntityMap.has(value.MACHINE_NAME)) {
								...
								vPersonelTracker.properties.operator_oid = ""+ vULTSEntityMap.get(value.MACHINE_NAME).PERSON_OID;
                                vPersonelTracker.properties.operator_name = "" + vULTSEntityMap.get(value.MACHINE_NAME).OPERATOR_NAME;
								vPersonelTracker.properties.employee_id = "" + vULTSEntityMap.get(value.MACHINE_NAME).EMPLOYEE_ID;
                            }
                    ...
                    for (const value of recordset) {
                        try {
							...
							if (vULTSPersonMap.has(vEntity.PERSON_OID)){
								vEntity.EMPLOYEE_ID = vULTSPersonMap.get(vEntity.PERSON_OID).EMPLOYEE_ID;
...
async function loadULTSPersonData() {
    if (!vLoadingULTSPersonData) {
                ...
                for (const recordset of result.recordsets) {
                    ...
                    for (const value of recordset) {
                        try {
                            ...
                            vPerson.EMPLOYEE_ID = parseInt(value.EMPLOYEE_ID);
							...
							vULTSPersonMapByEmployeeID.set(vPerson.EMPLOYEE_ID, vPerson);
                        } catch (err) {
        ...
        console.log("ULTS:loadULTSPersonData: FINISHED LOADING ULTS PERSON Rows.");
    };
...
async function loadZoneActivityData() {
    if (!vLoadingZoneActivityData) {
        ...
        try {
            var request = dbConn.request();
            let result = await request.query("SELECT * FROM ULTSZONE_ACTIVITY WHERE END_TIME IS NULL ORDER BY START_TIME ASC;");
            if (result !== null) {
                for (const recordset of result.recordsets) {
                    //vDATACMDEvent=[].concat(recordset);
                    ...
                    recordset.forEach((value, index) => {
                        try {
							...
							vZoneActivity.LAST_UPDATE = new Date(value.LAST_UPDATE);
                            //vZoneActivities.push(_.cloneDeep(vZoneActivity));
                    //        if (value.END_TIME != "NULL") {
                    //            vZoneActivity.ACTION = "Exited";
                    //            vZoneActivity.END_TIME = new Date(value.END_TIME);
                    ...
                    //        } else {
                                if (vZoneActivitiesCurrent.has(vZoneActivity.MACHINE_OID + "-" + vZoneActivity.ZONE_OID)) {
                                    ...
                                    if (vZoneCurrent.START_TIME < vZoneActivity.START_TIME) {
                                        vZoneActivitiesCurrent.set(vZoneActivity.MACHINE_OID + "-" + vZoneActivity.ZONE_OID, vZoneActivity);
                                    }
									console.warn("ULTS:loadZoneActivityData:["+vZoneActivity.MACHINE_NAME+"]["+vZoneActivity.ZONE_NAME+"]Should not have 2 records with NULL END_TIME...");
                                } else {
                                    vZoneActivitiesCurrent.set(vZoneActivity.MACHINE_OID + "-" + vZoneActivity.ZONE_OID, vZoneActivity);
                                }
                    //        }
...
var vSettingUpLocationEventSource = false
//Creates and listens to the event source for messages.
function setupLocationEventSource() {
    if (!vSettingUpLocationEventSource) {
        vSettingUpLocationEventSource = true;
		try{	
...
*/
    	...
    	vWatchDogId = setInterval(doTimeOutWatchDog, vWatchDogTimeOutInterval);
            ...
            console.error('ULTS:LOCATIONDATA:ERROR:' + JSON.stringify(err2) + '.');
        }
		vSettingUpLocationEventSource = false;
    }
}
...
async function doTimeOutWatchDog() {
    ...
    if (vWatchDogTimeOutCount > vWatchDogTimeOutResetValue) {
		vWatchDogTimeOutCount=0;
        console.error('ULTS:LOCATIONDATA:LOCATION WATCHDOG RESET!!!');
...
async function processLocationData(vLocationData) {
	...
	vZonePoly.features = [];
	if ( vZoneLookup ){
		vZonePoly = vZoneLookup.search(vLocX, vLocY, -1);
	}
	var vFMIZonePoly = {};
	vFMIZonePoly.features = [];
	if ( vFMIZoneLookup ){
    	try {
        	vFMIZonePoly = vFMIZoneLookup.search(vLocX, vLocY, -1);
    	} catch (err) {
    ...
    vPanelPoly.features = [];
    if ( vPanelLookup ){
        vPanelPoly = vPanelLookup.search(vLocX, vLocY, -1);
    }
    //var vPanelPoly = vPanelLookup.search(vLocX, vLocY, -1);
	var vAreaPoly = {};
    vAreaPoly.features = [];
    if ( vAreaLookup ){
        vAreaPoly = vAreaLookup.search(vLocX, vLocY, -1);
    }
    ...
    try {
        var vNewZones = [];
		//If Entity Has had locations before.
        if (vEntityZonesMap.has(vOID)) { 
            ...
            vNewZones = createZoneActivity(vLocationData, vZones, vOldZones);
            //console.log('ULTS:LOCATIONDATA:CREATEZONEACTIVITY:CURRENT['+JSON.stringify(vEntityZonesMap.get(vLocationData.properties.oid))+'].');
        } else {
			//if a new entity or after a restart lets use the database cache of last zones.
            var vNewZones = [];
            for (const zone of vZones) {
                if (vZoneActivitiesCurrent.has(vOID + "-" + zone.OID)) {
                    ...
                    zone.LAST_UPDATE = vZoneActivitiesCurrent.get(vOID + "-" + zone.OID).LAST_UPDATE;
		...
		//Lets store the Current Entities Zones.
		vEntityZonesMap.set(vOID, vNewZones);
		//Lets at the Zone Data to the Entities Locations.
...
function createZoneActivity(vLocationData, vZones, vOldZones) {
    ...
    vZones.forEach((zone) => {
        var vEntry = true;
        zone.LAST_UPDATE = new Date(vLocationData.properties.reported_at);
        vData.LAST_UPDATE = new Date(vLocationData.properties.reported_at);
        ...
        if (vEntry) {
			...
			//Lets update the timestamp on the zoneactivity lastupdate
			updateZoneActivity(_.cloneDeep(vData));	
		}
...
async function cleanupZoneActivity() {
    try {
		var request = dbConn.request();
		let result = await request
			.query("UPDATE ULTSZONE_ACTIVITY SET END_TIME=SYSDATETIMEOFFSET() WHERE END_TIME IS NULL;");
		 if (result !== null) {
            console.log('ULTS:CLEANUPZONEACTIVITY: Finished clearing all current active zone data SAVED RESULT:' + result.rowsAffected);
            //vDelayUpdateCount++;
        }
    } catch (err) {
        console.log('ULTS:CLEANUPZONEACTIVITY:SQL ERROR:' + err + '.');
    }
}

//Stores the FMI Zone data into the SQL Datbase.
async function updateZoneActivity(vData) {
    //lets first send it to our subscriptions
    try {
        var referenceId = "SubscribeZoneActivityCurrent";
        ...
        referenceId = "SubscribeZoneActivityCurrent-"+vData.ZONE_TYPE;
        if ( clientsByReferenceId[referenceId] ) {
            clientsByReferenceId[referenceId].forEach((client) => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ status: 'update', vData }));
        ...
        referenceId = "SubscribeZoneActivityCurrent-"+vData.ZONE_TYPE+"-"+vData.ZONE_SUBTYPE;
        if ( clientsByReferenceId[referenceId] ) {
            clientsByReferenceId[referenceId].forEach((client) => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ status: 'update', vData }));
        ...
        console.log('ULTS:UPDATEZONEACTIVITY:ERROR:Send to subsribers failed:'+ err +'.');
    }
    ...
    try {
        ...
        let result = await request
            ...
            .input("vLAST_UPDATE", sql.DateTimeOffset(3), vData.LAST_UPDATE)
            //.input( "vlast_update", sql.NVarChar ( 64 ), JSON.stringify(vData.properties.color) )
            .query("EXECUTE mrcZONEActivityUpdate @vENTITY_OID, @vZONE_OID, @vLAST_UPDATE");
        if (result !== null) {
            //console.log('ULTS:STOREZONEACTIVITY: ZONE ACTIVITY[' + vData.ENTITY_NAME + ']PROBLEM[' + vData.ZONE_NAME + '][' + vData.START_TIME.toISOString() + '][' + vData.END_TIME.toISOString() + ']['+JSON.stringify(vData.POSITION)+'] SAVED RESULT:' + result.rowsAffected);
            //vDelayUpdateCount++;
        }
    } catch (err) {
        console.log('ULTS:UPDATEZONEACTIVITY:SQL ERROR:' + err + ' ZONE ACTIVITY[' + vData.ENTITY_NAME + '][' + vData.ZONE_NAME + '].');
    }
}
...
async function storeZoneActivity(vData) {
	//lets first send it to our subscriptions
	try {
		var referenceId = "SubscribeZoneActivity";
		if ( clientsByReferenceId[referenceId] ) {
			clientsByReferenceId[referenceId].forEach((client) => {
				if (client.readyState === WebSocket.OPEN) {
					client.send(JSON.stringify({ status: 'update', vData }));
		...
		referenceId = "SubscribeZoneActivity-"+vData.ZONE_TYPE;
        if ( clientsByReferenceId[referenceId] ) {
            clientsByReferenceId[referenceId].forEach((client) => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ status: 'update', vData }));
		...
		referenceId = "SubscribeZoneActivity-"+vData.ZONE_TYPE+"-"+vData.ZONE_SUBTYPE;
		if ( clientsByReferenceId[referenceId] ) {
            clientsByReferenceId[referenceId].forEach((client) => {
                if (client.readyState === WebSocket.OPEN) {
                    client.send(JSON.stringify({ status: 'update', vData }));
    ...
    try {
        ...
        if (result !== null) {
			if ( vData.END_TIME === sql.NULL ){
            	console.log('ULTS:STOREZONEACTIVITY: ZONE ACTIVITY[' + vData.ENTITY_NAME + ']ENTERED[' + vData.ZONE_NAME + '][' + vData.START_TIME.toISOString() + '][NULL]['+JSON.stringify(vData.POSITION)+'] SAVED RESULT:' + result.rowsAffected);
			}else if ( vData.START_TIME === sql.NULL ){
            	console.log('ULTS:STOREZONEACTIVITY: ZONE ACTIVITY[' + vData.ENTITY_NAME + ']EXITED[' + vData.ZONE_NAME + '][NULL][' + vData.END_TIME.toISOString() + ']['+JSON.stringify(vData.POSITION)+'] SAVED RESULT:' + result.rowsAffected);
			}else{
				console.log('ULTS:STOREZONEACTIVITY: ZONE ACTIVITY[' + vData.ENTITY_NAME + ']PROBLEM[' + vData.ZONE_NAME + '][' + vData.START_TIME.toISOString() + '][' + vData.END_TIME.toISOString() + ']['+JSON.stringify(vData.POSITION)+'] SAVED RESULT:' + result.rowsAffected);
			}
            //vDelayUpdateCount++;
...
async function storeULTSEntity(vData) {
	console.log('ULTS:STOREULTSENTITY:Storing [' + vData.MACHINE_NAME + ']...');
    //Lets store it as the latest data for the current loader api calls.
    try {
        var request = dbConn.request();
        let result = await request
...
function makeDisplayName(vFullName){
	var vDisplayName="";
	if (vFullName.length < 9){
		vDisplayName=vFullName;
	}else{
		var vNameParts = vFullName.split(' ');
		if (vNameParts.length===1){
			vDisplayName=vFullName.substring(0,8);
		}else{
			var vStart = vNameParts[0].substring(0,1);
			var vEnd = vNameParts[vNameParts.length-1];
			if ( vNameParts[vNameParts.length-1].length > 7 ){
				vEnd = vNameParts[vNameParts.length-1].substring(0,7);
			}
			vDisplayName=vStart+vEnd;
		}
	}
	return vDisplayName;
}
...
async function storeULTSPerson(vData) {
    ...
    try {
        ...
        if (result !== null) {
			...
			vULTSPersonMapByEmployeeID.set(vData.EMPLOYEE_ID, vData);
            vDataStoreCount++;
        }
    } catch (err) {
...
function getEntitiesInZoneOID(vEntityToZones, vZoneOID) {
    var vReturnData = [];
    vZoneOIDs = vZoneOID.split(",");
    //console.log("ENTITYZONESMAP["+JSON.stringify(vEntityZonesMap)+"].");
    vEntityToZones.forEach((vZones, vKey) => {
        var vInclude = false;
        var vEntity = _.cloneDeep(vFLTLocationMap.get(vKey));
        if (vEntity) {
            ...
            if (Array.isArray(vZones)) {
                vZones.forEach((zone) => {
                    //console.log("ENTITYINZONE:NAME["+vEntity.properties.name+"]ZONE["+zone.NAME+"].");
                    vZoneOIDs.forEach((oid) => {
						try{
                        	if (zone.OID === parseInt(oid) || oid === "" || !oid) {
                            	vInclude = true;
                            	vEntity.ZONES.push(zone);
                        	}
						}catch(err){
							console.error("Cant parse OID int["+vZoneOID+"]["+oid+"].");
						}
                        //vEntity.ZONES.push(zone);
                    });
                });
    ...
    console.log("Before ZONENAME Filter(" + vZoneOID + ")(" + vEntityZonesMap.size + ")after(" + vReturnData.length + ").");
    return vReturnData;
}
...
function zonenameFilterData(data, vZoneName) {
    ...
    console.log("Before ZONE_NAME Filter(" + vZoneName + ")(" + data.length + ")after(" + vReturnData.length + ").");
    return vReturnData;
}
...
async function setupPositionPacketListener() {
    ...
    client.on('message', async function(message, remote) {
        ...
        for (const vPosPacket of vPosPackets) {
			...
			//}
            if (vPosPacket.is_infrastructure == 0 && vPosPacket.asset_name && vPosPacket.asset_name.length < 16 && vPosPacket.asset_id >= 0 && vPosPacket.position && vPosPacket.position[2] && vPosPacket.position[2] < 4000 && (vPosPacket.asset_id < 10000 || vPosPacket.asset_name.substring(0,3) == "CL_" ) && vPosPacket.asset_name.substring(0,2) != "B_") {
                ...
                if (vMOLocationNameMap.has(vPosPacket.asset_name)) {
                    ...
                    if (!vPosPacket.forward_heading) {
                        if ((vPersonelTracker.geometry.coordinates) && (vPosPacket.position)) {
							...
							var vOldHeading = vPersonelTracker.properties.heading;
							var vCurrentHeading = angleFromCoordinate(vOldPos[0], vOldPos[1], vNewPos[0], vNewPos[1]);
							var vAngleDiff = utils.angleDelta(vCurrentHeading, vOldHeading);
							if (Math.abs(vAngleDiff) > 90) {
                    			//It is facing the wrong way.
                    			vCurrentHeading = vCurrentHeading + 180;
                    			if (vCurrentHeading > 360) {
                        			vCurrentHeading = vCurrentHeading - 360;
                    			}
                			}

                            vPersonelTracker.properties.heading = vCurrentHeading;
                        }
                    } else {
                    ...
                    if (vULTSEntityMap.has(vPosPacket.asset_name)) {
						...
						vPersonelTracker.properties.employee_id = "" + vEntity.EMPLOYEE_ID;
                    ...
                    if (vFLTNameToClassMap.has(vPosPacket.asset_name)) {
						...
						//if ( vPosPacket.asset_name.substring(0,3) == "CL_" ){
    ...
    if (!vProcessingDelayedLocations) {
        ...
        for (const packet of vSortedDelayedLocations) {
            ...
            if (packet.sendTime < vUTCSec) {
                ...
                vFLTLocationMap.set(packet.properties.oid, packet);
                if (packet.geometry.coordinates) {
                    if (packet.geometry.coordinates.length === 3) {
...
async function doAssignment(vEntityID,vEmployeeID){
	console.log("UPDATEULTSENTITYASSIGNMENT:Assigning EntityID["+vEntityID+"] to EmployeeID["+vEmployeeID+"].");
	var vResult="";
//Lets do assignment
    if ( vULTSEntityMapByOID.has(vEntityID)){
            var vEntity = vULTSEntityMapByOID.get(vEntityID);
            var vPersonOID=0;
            if (vEmployeeID && vEmployeeID!=0) {
                if ( vULTSPersonMapByEmployeeID.has(vEmployeeID) ) {
                    vPersonOID=vULTSPersonMapByEmployeeID.get(vEmployeeID).OID;
					console.log("UPDATEULTSENTITYASSIGNMENT:Existing Person Assigning EntityID["+vEntityID+"] to PERSONOID["+vPersonOID+"].");
                }else{
                    //Lets create a new Person.
					console.log("UPDATEULTSENTITYASSIGNMENT:New Person Creating them First.");
                    var vPersonDetails = await loadPersonDetails(vEmployeeID);
                    if ( vPersonDetails.EMPLOYEE_ID && vPersonDetails.EMPLOYEE_ID != "" ) {
                        var vPerson = {};
                        vPerson.OID = vEmployeeID;
                        vPerson.ACTIVE = true;
                        vPerson.PERSON_NAME = ""+vPersonDetails.NAME;
                        vPerson.DISPLAY_NAME = makeDisplayName(vPersonDetails.NAME);
                        vPerson.EMPLOYEE_ID = vEmployeeID;
                        vPerson.CLASS_OID = 666666666;
                        vPerson.ROLE = "WORKER";
                        vPerson.ENTITYGROUPROLE_OID = parseInt(vEntity.ENTITYGROUPROLE_OID);
                        console.log("UPDATEULTSEntityAssignment:Creating New Person[" + JSON.stringify(vPerson) + "]");
						var vPersonResult = await storeULTSPerson(vPerson);
                        vResult = "Creating Person[" + JSON.stringify(vPersonResult)+ "] ";
                        vULTSPersonMap.set(vPerson.OID, vPerson);
                        vULTSPersonMapByEmployeeID.set(vPerson.EMPLOYEE_ID, vPerson);
						vPersonOID=vPerson.OID;
						console.log("UPDATEULTSENTITYASSIGNMENT:New Person Assigning EntityID["+vEntityID+"] to PERSONOID["+vPersonOID+"]DISPLAYNAME["+vPerson.DISPLAY_NAME+"].");
                    }else{
                        vResult="Unknown employee_id["+vEmployeeID+"].";
                    }
                }
            }else{
				console.log("UPDATEULTSENTITYASSIGNMENT:No Employee_id or 0["+vEmployeeID+"] clearing current assignment.");
			}
			...
			/*if ( vEntity.PERSON_OID === 0){
            	...
            	vULTSEntityMap.forEach((entity) => {
                	if (entity.PERSON_OID===vPersonOID){
                    	entity.PERSON_OID=0;
            			...
            			var vEntityResultClear=storeULTSEntity(entity);
                	}
            	});
				vEntity.OPERATOR_NAME=vULTSPersonMap.get(vPersonOID).DISPLAY_NAME;
			}*/
            vULTSEntityMap.set(vEntity.MACHINE_NAME, vEntity);
            vULTSEntityMapByOID.set(vEntity.OID, vEntity);
			var vEntityResult=await storeULTSEntity(vEntity);
            vResult = vResult + "Updating Entity[" + JSON.stringify(vEntityResult) + "]";
			console.log("UPDATEULTSENTITYASSIGNMENT:Assigned EntityID["+vEntityID+"]NAME["+vEntity.MACHINE_NAME+"]To PERSONOID["+vPersonOID+"]Result["+JSON.stringify(vEntityResult)+"].");
   	}else{
			console.log("UPDATEULTSENTITYASSIGNMENT:Unknown EntityID["+vEntityID+"].");
            vResult="Unknown entity_id["+vEntityID+"].";
   	}
	return vResult;	
}

async function clearEntityAssignments(vEntity){
    if ( vEntity.PERSON_OID === 0){
        vEntity.OPERATOR_NAME=vEntity.MACHINE_NAME;
    }else{
        //First let go through all Entities and remove this user.
        for ( const entity in vULTSEntityMap){
            if (entity.PERSON_OID===vEntity.PERSON_OID){
                ...
                vULTSEntityMap.set(entity.MACHINE_NAME, entity);
                vULTSEntityMapByOID.set(entity.OID, entity);
                var vEntityResult= await storeULTSEntity(entity);
            }
        };
        if ( vULTSPersonMap.has(vEntity.PERSON_OID)){
            vEntity.OPERATOR_NAME=vULTSPersonMap.get(vEntity.PERSON_OID).DISPLAY_NAME;
...
//Lets update a Persons Assingmend to a Tag.
app.get('/api/updateULTSEntityAssignment', (req, res) => {
	var vResult = {};
	if ( req.query.entity_id ) {
		vResult = doAssignment(parseInt(req.query.entity_id), parseInt(req.query.employee_id));
	}else{
		vResult="Must have entity_id.";
	}
    res.json(vResult); //{ id: vNewZone.id});
    console.log("UPDATEULTSENTITYASSIGNMENT:Result[" + JSON.stringify(vResult) + "]");
});
...
app.put('/api/updateULTSEntity', (req, res) => {
	...
	vEntity = clearEntityAssignments(vEntity);
/*	if ( vEntity.PERSON_OID === 0){
    	vEntity.OPERATOR_NAME=vEntity.MACHINE_NAME;
    }else{
        //First let go through all Entities and remove this user.
        vULTSEntityMap.forEach((entity) => {
        	if (entity.PERSON_OID===vEntity.PERSON_OID){
                ...
                vULTSEntityMap.set(entity.MACHINE_NAME, entity);
                vULTSEntityMapByOID.set(entity.OID, entity);
                var vEntityResult=storeULTSEntity(entity);
            }
        });
		if ( vULTSPersonMap.has(vEntity.PERSON_OID)){
        	vEntity.OPERATOR_NAME=vULTSPersonMap.get(vEntity.PERSON_OID).DISPLAY_NAME;
...
app.get('/api/getFLTAutoZoneEntitiesList', (req, res) => {
	...
	if ( req.query.zone_name ){
    	vReturnData = getEntitiesInZone(vTempMap, req.query.zone_name);
	}else if ( req.query.zone_oid ){
		vReturnData = getEntitiesInZoneOID(vTempMap, req.query.zone_oid);
	} 
    if (req.query.minlastupdate) {
        vReturnData = filterMinLastUpdate(vReturnData, req.query.minlastupdate);
    }
...
/*app.get('/api/getFLTAutoZoneActivities', (req, res) => {
    console.log("getFLTAutoZoneActivities requested data for Zone("+req.query.zone_name+")From("+vZoneActivities.length+").");
    var vReturnData = vZoneActivities;
    if (req.query.zone_name) {
        vReturnData = zonenameFilterData(vZoneActivities, req.query.zone_name);
    }
    vReturnData = filter.doFilters(vReturnData, req);
    console.log("getFLTAutoZoneActivities sending data ("+vReturnData.length+").");
