...
var waitFuncAreaEventSource = function() { return reconnectFrequencySecondsAreaEventSource * 1000 };
var waitFuncPanelEventSource = function() { return reconnectFrequencySecondsPanelEventSource * 1000 };
var waitFuncZoneEventSource = function() { return reconnectFrequencySecondsZoneEventSource * 1000 };
var waitFuncDelayEventSource = function() { return reconnectFrequencySecondsDelayEventSource * 1000 };
var waitFuncLocationEventSource = function() { return reconnectFrequencySecondsLocationEventSource * 1000 };
...
var reconnectFuncAreaEventSource = function() { setTimeout(tryToSetupFuncAreaEventSource, waitFuncAreaEventSource()) };
var reconnectFuncPanelEventSource = function() { setTimeout(tryToSetupFuncPanelEventSource, waitFuncPanelEventSource()) };
var reconnectFuncZoneEventSource = function() { setTimeout(tryToSetupFuncZoneEventSource, waitFuncZoneEventSource()) };
var reconnectFuncDelayEventSource = function() { setTimeout(tryToSetupFuncDelayEventSource, waitFuncDelayEventSource()) };
var reconnectFuncLocationEventSource = function() { setTimeout(tryToSetupFuncLocationEventSource, waitFuncLocationEventSource()) };
...
async function loadLocationDataHistory(){
        if(!vLoadingLocationDataHistory){
					...
					var vSendTime=new Date().getTime();
						...
						try{
							...
							vPersonelTracker.properties.reported_at = new Date(value.START_TIME).getTime();
                                			...
                                			vPersonelTracker.properties.position_source="mstarfleet";
                ...
                console.log("ULTS:loadLocationDataHistory: FINISHED LOADING ALL FMI ZONE Rows.");
                //vFMIZones.features=_.cloneDeep(vUpdateFeatures);
	};
...
function setupLocationEventSource() {
        ...
        vLocationEvtSource.on('update', function(e) {
			...
			var vMOHeading = -1;
			vMOLocationMap.forEach((mopacket) => {
					...
					if (mopacket.properties.heading!=0){
						...
						console.log('ULTS:LOCATIONDATA:UPDATE:HACK Replacing Heading with MO Heading.['+vLocationData.properties.name+']['+vLocationData.properties.heading+'].');
					}
				}
                        });
//				"features": [
...
async function storeZone(vData) {
        //vCurrentMachine.set(vData.name,vData);
			...
			console.log("STOREZONE:["+JSON.stringify(vData)+"].");
                        ...
                        console.log('FLTZONE:SQL ERROR:' + err + ' FMI ZONE['+ vData.id + '].');
                }
}
...
async function storeLocation(vData) {
        //Lets store it as the latest data for the current loader api calls.
                try {
                        ...
                        let result = await request
                                .input( "vmachine_oid", sql.BigInt , vData.id )
                                ...
                                .query( "EXECUTE mrcFLTLOCATIONMerge @vmachine_oid, @vstart_time, @vmachine_name, @vactive, @vclass_oid, @vfuel_level, @voperator_name, @vheading, @vsteering_angle, @vx, @vy, @vz, @vposition_source, @vUNIQUE_CONST");
                        if (result !== null) {
...
function getEntitiesInZone(vEntityToZones,vZoneName){
	...
	vEntityToZones.forEach((vZones,vKey) => {
		var vInclude=false;
		var vEntity = vFLTLocationMap.get(vKey);
		if (vEntity){
...
async function setupPositionPacketListener(){
	...
	//	console.log("ULTS:POSPACKET:POSPACKETSRECIEVED["+JSON.stringify(vPosPackets.asset_id)+"].");
		var vPersonelTracker = {};
		var vPPackets = [];
		...
		//vPosPackets.forEach(async (vPosPacket) => {
		    ...
		    if ( vPosPacket.is_infrastructure == 0 && vPosPacket.asset_name && vPosPacket.asset_name.length < 16 && vPosPacket.asset_id > 0 && vPosPacket.position && vPosPacket.position[2] && vPosPacket.position[2] < 4000 && vPosPacket.asset_id < 10000 ){
			//console.log("ULTS:POSPACKET:POSPACKETSRECIEVED:MOPACKET["+JSON.stringify(vPosPacket)+"].");
			if ( vMOLocationMap.has(vPosPacket.asset_id) ){
				vPersonelTracker = _.cloneDeep(vMOLocationMap.get(vPosPacket.asset_id));
				...
				vMOLocationMap.set(vPosPacket.asset_id,vPersonelTracker);
			}else{
				vPersonelTracker = {};
				vPersonelTracker.type = "Feature";
				vPersonelTracker.id = 0+vPosPacket.asset_id; //#HACK This should be mapped from MO to MStar.
				if (vPosPacket.position){
					vPersonelTracker.geometry = {};
					vPersonelTracker.geometry.type = "Point";
					vPersonelTracker.geometry.coordinates = _.cloneDeep(Array.from(vPosPacket.position));
			...
			if ( vPersonelTracker.properties.class_oid === 666666666){
				if ( !vPersonelTracker.properties.name.includes("UNKNOWN") ){
					vDelayedLocations.unshift(_.cloneDeep(vPersonelTracker));
				}
			...
			//else{
			//	updateLocationHeading(vPersonelTracker);
			//}
		    }
		};
/*	
...
/*app.get('/api/getMSLocation', (req,res)=> {
        //var vReturnData=utils.getArray(vFLTLocationMap);
        var vReturnData=vFLTLocationHistory;
	console.log("ULTS:getMSLocation:Getting Location history data["+vReturnData.length+"] now filtering...");
	console.log("ULTS:getMSLocation:DataExample["+JSON.stringify(vReturnData[0])+"].");
	vReturnData=filter.doFilters(vReturnData,req);
	console.log("ULTS:getMSLocation:After Filter["+vReturnData.length+"].");
	...
	/*if ( req.startdatetime && req.enddatetime ) {
		var vStartKey=(new Date(req.startdatetime)).getTime()/1000;
		var vEndKey=(new Date(req.startdatetime)).getTime()/1000;
		if ( vEndKey > vStartKey){
			var vKey=vStartKey;
			while (vKey < vEndKey){
				if (vFLTLocationHistory.has(vKey)){
					vFLTLocationHistory.get(vKey).forEach((data) => {
						vReturnData.push(data);
					});
				}
				vKey++;
			}
